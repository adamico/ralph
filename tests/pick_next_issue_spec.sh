...
test_root_config_generator() {
  local test_dir
  test_dir=$(mktemp -d)
  cd "$test_dir" || exit 1

  # Initialize a git repo for shell dominance detection
  git init -q .
  git config user.email "test@test.com"
  git config user.name "Test"

  # Test 1: Root config contains github backend
  local config
  config=$(BACKEND=github GH_REPO="" generate_root_config "." "myrepo" 2>&1)
  if echo "$config" | grep -q 'BACKEND="github"'; then
    PASS=$((PASS+1))
  else
    FAIL=$((FAIL+1))
    FAILED_NAMES+=("root_config: github backend")
    echo "FAIL: root_config: github backend"
  fi

  # Test 2: Root config has haiku model
  if echo "$config" | grep -q 'MODEL="haiku"'; then
    PASS=$((PASS+1))
  else
    FAIL=$((FAIL+1))
    FAILED_NAMES+=("root_config: haiku model")
    echo "FAIL: root_config: haiku model"
  fi

  # Test 3: Root config has host claude command
  if echo "$config" | grep -q 'CLAUDE_CMD="claude"'; then
    PASS=$((PASS+1))
  else
    FAIL=$((FAIL+1))
    FAILED_NAMES+=("root_config: host claude")
    echo "FAIL: root_config: host claude"
  fi

  # Test 4: Root config has placeholder test command
  if echo "$config" | grep -q 'TEST_CMD=":"'; then
    PASS=$((PASS+1))
  else
    FAIL=$((FAIL+1))
    FAILED_NAMES+=("root_config: placeholder test")
    echo "FAIL: root_config: placeholder test"
  fi

  # Test 5: Root config with shell scripts dominant gets shellcheck
  mkdir -p bin src
  touch bin/script1.sh bin/script2.sh bin/script3.sh
  touch src/file1.py src/file2.py
  git add -A
  git commit -q -m "test" 2>/dev/null || true
  config=$(BACKEND=github GH_REPO="" generate_root_config "." "myrepo" 2>&1)
  if echo "$config" | grep -q 'LINT_CMD="shellcheck"'; then
    PASS=$((PASS+1))
  else
    FAIL=$((FAIL+1))
    FAILED_NAMES+=("root_config: shell dominance")
    echo "FAIL: root_config: shell dominance"
  fi
